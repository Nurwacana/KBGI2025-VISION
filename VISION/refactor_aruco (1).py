...
class DataSender(threading.Thread):
    ...
    def run(self):
        ...
        while not self._stop_event.is_set():
            ...
            # Check if the new data is the same as the last sent data
...
class MarkerGUI:
    ...
    def start_detection(self):
        ...
        try:
            ...
            messagebox.showwarning("Input Error", "Calibration folder must be 78 or 90!")
            return
        ...
        # Build API URL with resolved IP
        # api_url = f"http://{server_ip}/detector-getaran/api/receive_camera_data.php"
        self.DataSenderThread = DataSender(camera_handler=self.camera_thread, server_address=server_address)
